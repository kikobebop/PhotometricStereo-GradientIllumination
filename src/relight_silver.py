...
from config import RESULTS_DIR, REF_IMAGE_PATH, NORMALS_PATH, MASK_PATH, SILVER_OUTPUT_DIR, GIF_PATH_SILVER
from light import estimate_light_direction, generate_equatorial_light_path
from render import relight_with_phong

GIF_PATH = GIF_PATH_SILVER
...
frames = []
silver_rgb = np.array([0.95, 0.96, 0.97], dtype=np.float32)
...
for idx, light_dir in enumerate(light_dirs):
    ...
    frame = relight_with_phong(normals, ref_img_rgb, light_dir, ref_dot, mask,
                               kd=0.01, ks=2.0, shininess=32, ambient=0.01,
                               material_color=silver_rgb, view_dir=np.array([0, -1, 0]))
